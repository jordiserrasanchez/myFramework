 ...
 * Filename ajaxaccesstypescontroller.php 
 * Encoding UTF-8
 */

/**
 * Documentació de la classe ajaxaccesstypescontroller.
 * @access public 
 ...
 * @package ajaxaccesstypescontroller
 * @subpackage ajax
 * @version 1.0.0
 */
final class ajaxaccesstypescontroller {
    
    /**  @var string $model Conté el model */
    private $model;
    
    /**  @var string $tipusAcces Conté l'objecte tipusacces */
    private $tipusacces;
    ...
    public function __construct() {
        ...
        /** crea l'objecte de tipus d'accés */
        $this->model = new tipusacces ( );

        /** executa el mètodo que recupera el número de tipus d'accés */
        $this->numRows = $this->model->retornaNumeroTipusAcces ( );
                
        if ( $this->numRows['TotalTipusAcces'] > 0 ) {/** si hi han tipus d'accés */
            $this->tpages = ceil ( $this->numRows['TotalTipusAcces'] / _NUM_ROWS_BY_PAGE_ );
        ...
        $this->tipusacces = $this->model->getLlistaTipusAcces ( $this->offset . ', ' . $this->rowsPerPage , $this->order );
    }
    ...
    */
    public function render ( ) {
        ...
        $html.= '            <th style="display:none">idTipusAcces</th>';
        $html.= '            <th>Codi</th>';
        $html.= '            <th>Descripció</th>';
        $html.= '        </tr>';
        $html.= '    </thead>';
        $html.= '    <tbody>';
                foreach ( $this->tipusacces as $dato ) { /** per cada tipus d'accés */
        $html.= '        <tr>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=AccessTypes&action=Delete" method="post">';
        $html.= "                    <input type=\"text\" name=\"idEliminar\" id=\"idEliminar\" value=\"" . $dato["idTipusAcces"] . "\" hidden>";
        ...
        $html.= '            </td>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=AccessTypes&action=Edit" method="post">';
        $html.= "                    <input type=\"text\" name=\"idModificar\" id=\"idModificar\" value=\"" . $dato["idTipusAcces"] . "\" hidden>";
        ...
        $html.= '                </form>';
        $html.= '            </td>';          
        $html.= "            <td  style='display:none'>" . $dato["idTipusAcces"] . "</td>";
