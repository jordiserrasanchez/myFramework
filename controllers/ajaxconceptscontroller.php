 ...
 * Filename ajaxconceptscontroller.php 
 * Encoding UTF-8
 */

/**
 * Documentació de la classe ajaxconceptscontroller.
 * @access public 
 ...
 * @version 1.0.0
 */
final class ajaxconceptscontroller {
    ...
    public function __construct() {
        ...
        /** crea l'objecte del concepte */
        $this->model = new concepte ( );

        /** executa el mètodo que recupera el número de conceptes */
        $this->numRows = $this->model->retornaNumeroConceptes ( );
                
        if ( $this->numRows['TotalConceptes'] > 0 ) {/** si hi han conceptes */
            $this->tpages = ceil ( $this->numRows['TotalConceptes'] / _NUM_ROWS_BY_PAGE_ );
        ...
        $this->order = "concepte asc";
        
        /** crida al mètode que recupera la llista de conceptes */
        $this->concepte = $this->model->getLlistaConceptes ( $this->offset . ', ' . $this->rowsPerPage , $this->order );
    }
    ...
    */
    public function render ( ) {
        ...
        $html.= '            <th style="display:none">idConcepte</th>';
        $html.= '            <th>Concepte</th>';
        $html.= '            <th>Import U.</th>';
        $html.= '            <th>Actiu</th>';
        $html.= '        </tr>';
        $html.= '    </thead>';
        $html.= '    <tbody>';
                foreach ( $this->concepte as $dato ) { /** per cada concepte */
        $html.= '        <tr>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=Concepts&action=Delete" method="post">';
        $html.= "                    <input type=\"text\" name=\"idEliminar\" id=\"idEliminar\" value=\"" . $dato["idConcepte"] . "\" hidden>";
        ...
        $html.= '            </td>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=Concepts&action=Edit" method="post">';
        $html.= "                    <input type=\"text\" name=\"idModificar\" id=\"idModificar\" value=\"" . $dato["idConcepte"] . "\" hidden>";
        ...
        $html.= '                </form>';
        $html.= '            </td>';          
        $html.= "            <td  style='display:none'>" . $dato["idConcepte"] . "</td>";
        $html.= "            <td>" . $dato["concepte"] . "</td>";
        $import = str_replace ( '.' , ',' , $dato["importUnitari"] );
