 ...
 * Filename ajaxaccessresultscontroller.php 
 * Encoding UTF-8
 */

/**
 * Documentació de la classe ajaxaccessresultscontroller.
 * @access public 
 ...
 * @package ajaxaccessresultscontroller
 * @subpackage ajax
 * @version 1.0.0
 */
final class ajaxaccessresultscontroller {
    
    /**  @var string $model Conté el model */
    private $model;
    
    /**  @var string $resultatacces Conté l'objecte resultatacces */
    private $resultatacces;
    ...
    public function __construct() {
        ...
        /** crea l'objecte de resultat d'accés */
        $this->model = new resultatacces ( );

        /** executa el mètodo que recupera el número de resultat d'accés */
        $this->numRows = $this->model->retornaNumeroResultatsAcces ( );
                
        if ( $this->numRows['TotalResultatAcces'] > 0 ) {/** si hi han resultats d'accés */
            $this->tpages = ceil ( $this->numRows['TotalResultatAcces'] / _NUM_ROWS_BY_PAGE_ );
        ...
        $this->order = "descripcio asc";
        
        /** crida al mètode que recupera la llista de resultats d'accés  */
        $this->resultatacces = $this->model->getLlistaResultatAcces ( $this->offset . ', ' . $this->rowsPerPage , $this->order );
    }
    ...
    */
    public function render ( ) {
        ...
        $html.= '            <th style="display:none">idResultatAcces</th>';
        $html.= '            <th>Codi</th>';
        $html.= '            <th>Descripció</th>';
        $html.= '        </tr>';
        $html.= '    </thead>';
        $html.= '    <tbody>';
                foreach ( $this->resultatacces as $dato ) { /** per cada tipus d'accés */
        $html.= '        <tr>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=AccessResults&action=Delete" method="post">';
        $html.= "                    <input type=\"text\" name=\"idEliminar\" id=\"idEliminar\" value=\"" . $dato["idResultatAcces"] . "\" hidden>";
        ...
        $html.= '            </td>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=AccessResults&action=Edit" method="post">';
        $html.= "                    <input type=\"text\" name=\"idModificar\" id=\"idModificar\" value=\"" . $dato["idResultatAcces"] . "\" hidden>";
        ...
        $html.= '                </form>';
        $html.= '            </td>';          
        $html.= "            <td  style='display:none'>" . $dato["idResultatAcces"] . "</td>";
        $html.= "            <td class=\"text-center\">" . $dato["codi"] . "</td>";
        $html.= "            <td>" . $dato["descripcio"] . "</td>";
        $html.= '        </tr>';
                }
        $html.= '    </tbody>';
        $html.= '</table>';
