 ...
 * Filename ajaxcomponentscontroller.php 
 * Encoding UTF-8
 */

/**
 * Documentació de la classe ajaxcomponentscontroller.
 * @access public 
 ...
 * @package ajaxcomponentscontroller
 * @subpackage ajax
 * @version 1.0.0
 */
final class ajaxcomponentscontroller {
    
    /**  @var string $model Conté el model */
    private $model;
    
    /**  @var string $concepte Conté l'objecte component */
    private $component;
    ...
    public function __construct() {
        ...
        /** crea l'objecte del component */
        $this->model = new component ( );

        /** executa el mètodo que recupera el número de components */
        $this->numRows = $this->model->retornaNumeroComponents ( );
                
        if ( $this->numRows['TotalComponents'] > 0 ) {/** si hi han components */
            $this->tpages = ceil ( $this->numRows['TotalComponents'] / _NUM_ROWS_BY_PAGE_ );
        ...
        $this->order = "m.modul, c.component ASC";
        
        /** crida al mètode que recupera la llista de components */
        $this->component = $this->model->getLlistaComponents ( $this->offset . ', ' . $this->rowsPerPage , $this->order );
    ...
    */
    public function render ( ) {
        ...
        $html.= '            <th style="display:none;">idComponent</th>';
        $html.= '            <th>Mòdul</th>';
        ...
        $html.= '    <tbody>';
        $i = 0;
                foreach ( $this->component as $dato ) { /** per cada component */
        $html.= '        <tr>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=Components&action=Delete" method="post">';
        $html.= "                    <input type=\"text\" name=\"idEliminar\" id=\"idEliminar\" value=\"" . $dato["idComponent"] . "\" hidden>";
        ...
        $html.= '            </td>';
        $html.= '            <td>';
        $html.= '                <form name="form" action="index.php?controlador=Components&action=Edit" method="post">';
        $html.= "                    <input type=\"text\" name=\"idModificar\" id=\"idModificar\" value=\"" . $dato["idComponent"] . "\" hidden>";
        ...
        $html.= '                </form>';
        $html.= '            </td>';          
        $html.= "            <td style=\"display:none;\">" . $dato["idComponent"] . "</td>";
        $html.= "            <td>" . $dato["modul"] . "</td>";
        $html.= "            <td>" . $dato["component"] . "</td>";
        $html.= "        </tr>";
        $i++;        
